...
def KitModel(weight_file=None):
    ...
    # conv2
    with tf.name_scope('block_2') as scope:
    ...
    # conv3
    with tf.name_scope('block_3') as scope:
    ...
    # conv4
    with tf.name_scope('block_4') as scope:
    ...
    # conv5
    with tf.name_scope('block_5') as scope:
    ...
    # conv1-side
    with tf.name_scope('block_6') as scope:
        score_dsn1 = convolution(relu1_2, group=1, strides=[
                                 1, 1], padding='VALID', name='score-dsn1')
        sigmoid_dsn1 = tf.sigmoid(score_dsn1, name='sigmoid-dsn1')

    # conv2-side
    with tf.name_scope('block_7') as scope:
        score_dsn2 = convolution(relu2_2, group=1, strides=[
                                 1, 1], padding='VALID', name='score-dsn2')
        upsample_2 = convolution_transpose(score_dsn2, output_shape=[1, tf.shape(data)[1], tf.shape(data)[2], 1], strides=[
                                           1, 2, 2, 1], padding='SAME', name='upsample_2')
        sigmoid_dsn2 = tf.sigmoid(upsample_2, name='sigmoid-dsn2')

    # conv3-side
    with tf.name_scope('block_8') as scope:
        score_dsn3 = convolution(relu3_3, group=1, strides=[
                                 1, 1], padding='VALID', name='score-dsn3')
        upsample_4 = convolution_transpose(score_dsn3, output_shape=[1, tf.shape(data)[1], tf.shape(data)[2], 1], strides=[
                                           1, 4, 4, 1], padding='SAME', name='upsample_4')
        sigmoid_dsn3 = tf.sigmoid(upsample_4, name='sigmoid-dsn3')

    # conv4-side
    with tf.name_scope('block_9') as scope:
        score_dsn4 = convolution(relu4_3, group=1, strides=[
                                 1, 1], padding='VALID', name='score-dsn4')
        upsample_8 = convolution_transpose(score_dsn4, output_shape=[1, tf.shape(data)[1], tf.shape(data)[2], 1], strides=[
                                           1, 8, 8, 1], padding='SAME', name='upsample_8')
        sigmoid_dsn4 = tf.sigmoid(upsample_8, name='sigmoid-dsn4')

    # conv5-side
    with tf.name_scope('block_10') as scope:
        score_dsn5 = convolution(relu5_3, group=1, strides=[
                                 1, 1], padding='VALID', name='score-dsn5')
        upsample_16 = convolution_transpose(score_dsn5, output_shape=[1, tf.shape(data)[1], tf.shape(data)[2], 1], strides=[
                                            1, 16, 16, 1], padding='SAME', name='upsample_16')
        sigmoid_dsn5 = tf.sigmoid(upsample_16, name='sigmoid-dsn5')

    # concatenate

    concat = tf.concat([score_dsn1, upsample_2, upsample_4,
                        upsample_8, upsample_16], 3, name='concat')

    with tf.name_scope('output_block') as scope:
